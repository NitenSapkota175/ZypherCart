...
def CartCheckout(request):
    ...
    if request.method == "POST":
        ...
        total_amount = cart_items.aggregate(total=Sum("price"))["total"]
        payment_method = request.POST.get("payment_method")
...
def PaymentSuccessView(request):
    if request.method == "POST":
        try:
            ...
            logger.info(
                f"Payment ID: {razorpay_payment_id}, Order ID: {razorpay_order_id}, Signature: {razorpay_signature}"
            )
